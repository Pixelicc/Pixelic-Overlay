  ...
  init: (UUID: string) => {
    try {
      ...
      const setActivity = async () => {
        axios
          .get(`https://api.pixelic.de/v2/pixelic-overlay/proxy/hypixel/player/${UUID}`, { headers: { "X-API-Key": dataStore.get("pixelicKey"), "User-Agent": `Pixelic-Overlay/${PackageJSON.version} (DiscordRPC)` } })
          .then(async (res) => {
